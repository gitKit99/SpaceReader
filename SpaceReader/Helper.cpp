...
bool Helper::getCloudAndNormals(const std::vector<Vertex>& points, pcl::PointCloud<pcl::PointNormal>::Ptr& _cloud, const double _estimationRadius)
{
	pcl::PointCloud<pcl::PointXYZ>::Ptr dataXYZ(new pcl::PointCloud<pcl::PointXYZ>());
	for (const Vertex& point : points) {
		dataXYZ->push_back(pcl::PointXYZ(point.pos.x, point.pos.y, point.pos.z));
	}
	Helper::removeNANs(dataXYZ);
	pcl::PointCloud<pcl::Normal>::Ptr normals = Helper::getNormals(dataXYZ, _estimationRadius);
